...
<div class="max-w-5xl">
        ...
        <div style="display: grid; grid-template-columns: 1fr; gap: 30px;" class="lg:grid-cols-3 lg:items-start">
                ...
                <div class="cs-card" style="padding: 24px; display: flex; flex-direction: column; gap: 20px;">
                    <div style="display: flex; align-items: center; justify-content: space-between;">
                        <div>
                            ...
                            <p style="font-size: 12px; color: var(--slate-400); margin-top: 2px;">Manage core recipe title, description, and times</p>
                        </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="title" x-model="title" required class="cs-input">
                    </div>

                    <div>
                        <label class="cs-label">Excerpt</label>
                        <textarea name="excerpt" x-model="excerpt" rows="2" class="cs-input" style="resize: none;"></textarea>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr);" class="sm:grid-cols-5 gap-4">
                        ...
                        <div>
                            ...
                            <select name="status" required class="cs-input" style="-webkit-appearance: none; background-color: var(--slate-50);">
                                ...
                                <option value="review" {{ old('status', $recipe->status) === 'review' ? 'selected' : '' }}>Review</option>
                ...
                <div class="cs-card" style="padding: 24px;">
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
                        <div>
                            ...
                            <p style="font-size: 12px; color: var(--slate-400); margin-top: 2px;">Add, edit, or remove ingredients for portion scaling</p>
                        </div>
                ...
                <div class="cs-card" style="padding: 24px;">
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
                        <div>
                            ...
                            <p style="font-size: 12px; color: var(--slate-400); margin-top: 2px;">Sequence of steps to prepare the meal</p>
                        </div>
            ...
            <div style="display: flex; flex-direction: column; gap: 30px;">
                ...
                <div class="cs-card" style="padding: 24px; display: flex; flex-direction: column; gap: 16px;">
                    ...
                    <div style="position: relative; border-radius: 12px; overflow: hidden; border: 1px solid var(--slate-200); aspect-ratio: 16/10;">
                        <img src="{{ $recipe->cover_image }}" style="width: 100%; height: 100%; object-fit: cover;" alt="Current cover">
                        <div style="position: absolute; bottom: 8px; left: 8px; padding: 4px 8px; border-radius: 6px; background: rgba(0,0,0,0.6); color: #fff; font-size: 10px; font-weight: 600;">Current Hero</div>
                    </div>
                    @endif

                    <div>
                        <label class="cs-label">Upload New Hero Image</label>
                    ...
                    <div>
                        ...
                        <input type="text" name="cover_image_url" value="{{ old('cover_image_url', $recipe->cover_image) }}" placeholder="https://unsplash.com/..." class="cs-input">
                    </div>
                </div>
                ...
                <div class="cs-card" style="padding: 24px; display: flex; flex-direction: column; gap: 16px;">
                    ...
                    <div>
                        ...
                        <input type="text" name="video_url" value="{{ old('video_url', $recipe->video_url) }}" placeholder="e.g. https://www.youtube.com/watch?v=..." class="cs-input">
                    </div>
                </div>
                ...
                <div class="cs-card" style="padding: 24px; display: flex; flex-direction: column; gap: 16px;">
                    ...
                    {{-- Current Gallery --}}
                    @php $gallery = $recipe->getMedia('gallery'); @endphp
                    @if($gallery->count() > 0)
                    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px;">
                        @foreach($gallery as $media)
                        <div style="position: relative; border-radius: 8px; overflow: hidden; border: 1px solid var(--slate-200); aspect-ratio: 1/1;" class="group">
                            <img src="{{ $media->getUrl() }}" style="width: 100%; height: 100%; object-fit: cover;" alt="Gallery">
                            <label style="position: absolute; top: 4px; right: 4px; display: flex; align-items: center; justify-content: center; width: 20px; height: 20px; border-radius: 4px; background: rgba(239,68,68,0.9); cursor: pointer; color: #fff; font-size: 10px;" title="Delete image">
                                <input type="checkbox" name="delete_media[]" value="{{ $media->id }}" style="display: none;" onchange="this.parentElement.style.background = this.checked ? '#374151' : 'rgba(239,68,68,0.9)'">
                    ...
                    @else
                    <div style="padding: 16px; border: 1px dashed var(--slate-200); border-radius: 12px; text-align: center; font-size: 12px; color: var(--slate-400);">
    ...
    {{-- Delete Button Form (Separated to avoid form nesting issues) --}}
    <div style="margin-top: -50px; display: inline-block;">
        <form method="POST" action="{{ route('admin.recipes.destroy', $recipe) }}" 
              onsubmit="return confirm('Permanently delete this recipe? This action cannot be undone.')">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn-danger" style="padding: 10px 18px; font-size: 13px;">
                Delete Recipe
            </button>
        </form>
    </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('recipeForm', () => ({
        title: {!! json_encode(old('title', $recipe->title)) !!},
        excerpt: {!! json_encode(old('excerpt', $recipe->excerpt)) !!},
        prep_time: {{ old('prep_time_minutes', $recipe->prep_time_minutes) }},
        cook_time: {{ old('cook_time_minutes', $recipe->cook_time_minutes) }},
        servings: {{ old('servings', $recipe->servings) }},
        description_html: {!! json_encode(old('description_html', $recipe->description_html)) !!},
        ...
        async generateFromAI() {
            ...
            try {
                ...
                alert('Recipe updated from AI successfully!');
            } catch (error) {
