        ...
        <form method="POST" action="{{ route('admin.categories.update', $category) }}">
            ...
            <div style="display: flex; flex-direction: column; gap: 20px;">
                <div>
                    ...
                    <p style="font-size: 12px; color: var(--slate-400); margin-top: 2px;">Modify category details</p>
                </div>

                <div>
                    <label class="cs-label">Category Name *</label>
                    <input type="text" name="name" value="{{ old('name', $category->name) }}" required placeholder="e.g. Desserts" class="cs-input">
                </div>

                <div>
                    <label class="cs-label">Custom Slug (optional)</label>
                    <input type="text" name="slug" value="{{ old('slug', $category->slug) }}" placeholder="e.g. desserts-and-sweets" class="cs-input">
